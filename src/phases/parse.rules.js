...
module.exports = parser.createLanguage({
  expression: rules =>
    parser.alt(rules.stringSingleQuote, rules.stringDoubleQuote, rules.number, rules.symbol, rules.list),
  ...
  number: () =>
    parser
      ...
      .map(Number)
      .desc('number'),
  list: rules => rules.expression.trim(parser.optWhitespace).many().wrap(parser.string('('), parser.string(')')),
